...
contract CacheManager is Initializable, DelegateCallAware {
  ...
  ArbOwnerPublic internal constant ARB_OWNER_PUBLIC =
    ArbOwnerPublic(address(0x6b));
  ...
  function sweepFunds() external {
    (
      bool success,
      bytes memory data // solhint-disable-next-line avoid-low-level-calls
    ) = ARB_OWNER_PUBLIC.getNetworkFeeAccount().call{
